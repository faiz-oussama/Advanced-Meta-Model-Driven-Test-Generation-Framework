package com.univade.TU.controller;

import com.univade.TU.testdata.AddressTestDataBuilder;
import com.univade.TU.dto.AddressDto;


import com.univade.TU.entity.Address;


import com.univade.TU.controller.AddressController;
import com.univade.TU.service.AddressService;
import com.univade.TU.exception.EntityNotFoundException;
import com.univade.TU.exception.BadRequestException;

import com.fasterxml.jackson.databind.ObjectMapper;
import org.junit.jupiter.api.*;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.WebMvcTest;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.autoconfigure.jdbc.AutoConfigureTestDatabase;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.test.web.client.TestRestTemplate;
import org.springframework.context.annotation.Import;
import org.springframework.http.*;
import org.springframework.security.test.context.support.WithMockUser;
import org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.http.ResponseEntity;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.test.web.servlet.MvcResult;

import java.lang.reflect.Field;
import java.util.ArrayList;
import jakarta.validation.constraints.Size;
import jakarta.validation.constraints.Min;
import jakarta.validation.constraints.Max;
import jakarta.validation.constraints.Email;
import jakarta.validation.constraints.Pattern;
import jakarta.validation.constraints.NotNull;
import jakarta.validation.constraints.NotBlank;

import jakarta.validation.ConstraintViolation;
import jakarta.validation.ConstraintViolationException;
import jakarta.validation.Validator;
import java.util.Arrays;
import java.util.List;
import java.util.Optional;
import java.util.Set;

import com.univade.TU.generator.model.ControllerValidationRule;

import static org.assertj.core.api.Assertions.*;
import static org.mockito.ArgumentMatchers.*;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.anyString;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.*;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;
import static org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.*;
import static org.hamcrest.Matchers.*;
import static org.assertj.core.api.Assertions.*;

@WebMvcTest(AddressController.class)
@AutoConfigureTestDatabase(replace = AutoConfigureTestDatabase.Replace.NONE)
@Import(com.univade.TU.config.TestSecurityConfig.class)
@WithMockUser
@DisplayName("Address Controller Tests")
class AddressControllerTest {


    @Autowired
    private MockMvc mockMvc;

    @MockBean
    private AddressService addressService;

    @Autowired
    private ObjectMapper objectMapper;

    @BeforeEach
    void setUp() {
        reset(addressService);
    }

    private Address createAddress() {
        return AddressTestDataBuilder.aValidAddress().build();
    }

    private AddressDto createAddressDto() {
        Address entity = createAddress();
        return AddressDto.builder()
                .street(entity.getStreet())
                .city(entity.getCity())
                .zipCode(entity.getZipCode())
                .build();
    }



    private Address createMockEntity() {
        Address mockEntity = createAddress();
        mockEntity.setId(1L);
        return mockEntity;
    }

    @Nested
    @DisplayName("CRUD Operations")
    class CrudOperations {

        @Test
        @DisplayName("Should create Address successfully with 201 status")
        void createAddressShouldReturnCreated() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            responseDto.setId(1L);

            Address mockEntity = createAddress();
            mockEntity.setId(1L);
            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.id").value(1));

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should get Address by ID successfully with 200 status")
        void getAddressByIdShouldReturnOk() throws Exception {
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            Address mockEntity = createAddress();
            mockEntity.setId(addressId);
            when(addressService.getAddressById(addressId)).thenReturn(createMockEntity());

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE))
                    .andExpect(jsonPath("$.id").value(1));

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should return 404 when Address not found")
        void getAddressByIdShouldReturn404WhenNotFound() throws Exception {
            Long addressId = 999L;

            when(addressService.getAddressById(addressId))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + addressId));

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.message").value("Address not found with id: " + addressId))
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should update Address successfully with 200 status")
        void updateAddressShouldReturnOk() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            Address mockEntity = createAddress();
            mockEntity.setId(addressId);
            when(addressService.updateAddress(eq(addressId), any(Address.class)))
                    .thenReturn(createMockEntity());

            mockMvc.perform(put("/api/addresses/{id}", addressId)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE))
                    .andExpect(jsonPath("$.id").value(1));

            verify(addressService).updateAddress(eq(addressId), any(Address.class));
        }

        @Test
        @DisplayName("Should delete Address successfully with 204 status")
        void deleteAddressShouldReturnNoContent() throws Exception {
            Long addressId = 1L;

            doNothing().when(addressService).deleteAddress(addressId);

            mockMvc.perform(delete("/api/addresses/{id}", addressId)
                    .with(csrf()))
                    .andExpect(status().isNoContent())
                    .andExpect(content().string(""));

            verify(addressService).deleteAddress(addressId);
        }

        @Test
        @DisplayName("Should get all Addresss successfully with 200 status")
        void getAllAddresssShouldReturnOk() throws Exception {
            AddressDto addressDto1 = createAddressDto();
            AddressDto addressDto2 = createAddressDto();
            addressDto1.setId(1L);
            addressDto2.setId(2L);

            List<AddressDto> addressDtoList = Arrays.asList(addressDto1, addressDto2);

            Address entity1 = createAddress();
            Address entity2 = createAddress();
            entity1.setId(1L);
            entity2.setId(2L);
            List<Address> entityList = Arrays.asList(entity1, entity2);
            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE))
                    .andExpect(jsonPath("$", hasSize(2)))
                    .andExpect(jsonPath("$[0].id").value(1))
                    .andExpect(jsonPath("$[1].id").value(2));

            verify(addressService).getAllAddresss();
        }
    }

    @Nested
    @DisplayName("Dynamic Validation Tests")
    class DynamicValidationTests {

        @Test
        @DisplayName("Should dynamically test all validation constraints from DTO")
        void testDynamicValidationConstraints() throws Exception {
            List<ControllerValidationRule> rules = createValidationRules();

            assertThat(rules).isNotEmpty();

            for (ControllerValidationRule rule : rules) {
                AddressDto dto = createAddressDto();

                setInvalidValueOnDto(dto, rule);
                mockMvc.perform(post("/api/addresses")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dto)))
                        .andExpect(status().isBadRequest())
                        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                        .andExpect(jsonPath("$.status").value(400))
                        .andExpect(jsonPath("$.message").exists())
                        .andExpect(jsonPath("$.timestamp").exists());
            }

            verify(addressService, never()).createAddress(any(Address.class));
        }

        private List<ControllerValidationRule> createValidationRules() {
            List<ControllerValidationRule> rules = new ArrayList<>();

            // Should return 400 when zipCode is blank
            rules.add(ControllerValidationRule.builder()
                    .testName("createAddressWithBlankZipCodeShouldReturnBadRequest")
                    .attributeName("zipCode")
                    .validationType("NotBlank")
                    .description("Should return 400 when zipCode is blank")
                    .expectedHttpStatus("400")
                    .message("Zip code cannot be blank")
                    .invalidValue("")
                    .build());

            // Should return 400 when zipCode has invalid size
            rules.add(ControllerValidationRule.builder()
                    .testName("createAddressWithInvalidSizeZipCodeShouldReturnBadRequest")
                    .attributeName("zipCode")
                    .validationType("Size")
                    .description("Should return 400 when zipCode has invalid size")
                    .expectedHttpStatus("400")
                    .message("Zip code must be between 3 and 10 characters")
                    .minValue(3)
                    .maxValue(10)
                    .invalidValue("aaaaaaaaaaa")
                    .build());

            // Should return 400 when city is blank
            rules.add(ControllerValidationRule.builder()
                    .testName("createAddressWithBlankCityShouldReturnBadRequest")
                    .attributeName("city")
                    .validationType("NotBlank")
                    .description("Should return 400 when city is blank")
                    .expectedHttpStatus("400")
                    .message("City cannot be blank")
                    .invalidValue("")
                    .build());

            // Should return 400 when city has invalid size
            rules.add(ControllerValidationRule.builder()
                    .testName("createAddressWithInvalidSizeCityShouldReturnBadRequest")
                    .attributeName("city")
                    .validationType("Size")
                    .description("Should return 400 when city has invalid size")
                    .expectedHttpStatus("400")
                    .message("City must be between 2 and 100 characters")
                    .minValue(2)
                    .maxValue(100)
                    .invalidValue("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
                    .build());

            // Should return 400 when street is blank
            rules.add(ControllerValidationRule.builder()
                    .testName("createAddressWithBlankStreetShouldReturnBadRequest")
                    .attributeName("street")
                    .validationType("NotBlank")
                    .description("Should return 400 when street is blank")
                    .expectedHttpStatus("400")
                    .message("Street cannot be blank")
                    .invalidValue("")
                    .build());

            // Should return 400 when street has invalid size
            rules.add(ControllerValidationRule.builder()
                    .testName("createAddressWithInvalidSizeStreetShouldReturnBadRequest")
                    .attributeName("street")
                    .validationType("Size")
                    .description("Should return 400 when street has invalid size")
                    .expectedHttpStatus("400")
                    .message("Street must be between 5 and 150 characters")
                    .minValue(5)
                    .maxValue(150)
                    .invalidValue("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
                    .build());

            return rules;
        }

        private void setInvalidValueOnDto(AddressDto dto, ControllerValidationRule rule) throws Exception {
            Field field = AddressDto.class.getDeclaredField(rule.getAttributeName());
            field.setAccessible(true);

            if (rule.isNotNull()) {
                field.set(dto, null);
            } else if (rule.isNotBlank()) {
                field.set(dto, "");
            } else if (rule.isSize() && rule.getMaxValue() != null) {
                int maxLength = ((Number) rule.getMaxValue()).intValue();
                field.set(dto, "a".repeat(maxLength + 1));
            } else if (rule.isMin() && rule.getConstraintValue() != null) {
                Number minValue = (Number) rule.getConstraintValue();
                setNumericFieldValue(field, dto, minValue.intValue() - 1);
            } else if (rule.isMax() && rule.getConstraintValue() != null) {
                Number maxValue = (Number) rule.getConstraintValue();
                setNumericFieldValue(field, dto, maxValue.intValue() + 1);
            } else if (rule.isEmail()) {
                field.set(dto, "invalid-email");
            } else if (rule.isPattern()) {
                field.set(dto, "invalid-pattern-value");
            }
        }

        private void setNumericFieldValue(Field field, AddressDto dto, int value) throws Exception {
            Class<?> fieldType = field.getType();
            if (fieldType == Integer.class || fieldType == int.class) {
                field.set(dto, value);
            } else if (fieldType == Long.class || fieldType == long.class) {
                field.set(dto, (long) value);
            } else if (fieldType == Short.class || fieldType == short.class) {
                field.set(dto, (short) value);
            } else if (fieldType == Byte.class || fieldType == byte.class) {
                field.set(dto, (byte) value);
            } else {
                field.set(dto, value);
            }
        }

        @Test
        @DisplayName("Should return 400 when multiple validation errors occur")
        void createAddressWithMultipleValidationErrorsShouldReturnBadRequest() throws Exception {
            AddressDto requestDto = AddressDto.builder().build();

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any(Address.class));
        }

    }

    @Nested
    @DisplayName("Exception Handling Tests")
    class ExceptionHandlingTests {

        @Test
        @DisplayName("Should return 404 when entity not found on GET")
        void getAddressByIdShouldReturn404WhenEntityNotFound() throws Exception {
            Long nonExistentId = 999L;

            when(addressService.getAddressById(nonExistentId))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + nonExistentId));

            mockMvc.perform(get("/api/addresses/{id}", nonExistentId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.error").value("Not Found"))
                    .andExpect(jsonPath("$.message").value("Address not found with id: " + nonExistentId))
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + nonExistentId))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).getAddressById(nonExistentId);
        }

        @Test
        @DisplayName("Should return 404 when entity not found on UPDATE")
        void updateAddressShouldReturn404WhenEntityNotFound() throws Exception {
            Long nonExistentId = 999L;
            AddressDto requestDto = createAddressDto();

            when(addressService.updateAddress(eq(nonExistentId), any(Address.class)))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + nonExistentId));

            mockMvc.perform(put("/api/addresses/{id}", nonExistentId)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.error").value("Not Found"))
                    .andExpect(jsonPath("$.message").value("Address not found with id: " + nonExistentId))
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + nonExistentId))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).updateAddress(eq(nonExistentId), any(Address.class));
        }

        @Test
        @DisplayName("Should return 404 when entity not found on DELETE")
        void deleteAddressShouldReturn404WhenEntityNotFound() throws Exception {
            Long nonExistentId = 999L;

            doThrow(new EntityNotFoundException("Address not found with id: " + nonExistentId))
                    .when(addressService).deleteAddress(nonExistentId);

            mockMvc.perform(delete("/api/addresses/{id}", nonExistentId)
                    .with(csrf()))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.error").value("Not Found"))
                    .andExpect(jsonPath("$.message").value("Address not found with id: " + nonExistentId))
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + nonExistentId))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).deleteAddress(nonExistentId);
        }

        @Test
        @DisplayName("Should return 500 when service throws unexpected exception")
        void createAddressShouldReturn500OnUnexpectedException() throws Exception {
            AddressDto requestDto = createAddressDto();

            when(addressService.createAddress(any(Address.class)))
                    .thenThrow(new RuntimeException("Unexpected database error"));

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isInternalServerError())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(500))
                    .andExpect(jsonPath("$.error").value("Internal Server Error"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 400 when service throws BadRequestException")
        void createAddressShouldReturn400OnBadRequestException() throws Exception {
            AddressDto requestDto = createAddressDto();

            when(addressService.createAddress(any(Address.class)))
                    .thenThrow(new BadRequestException("Invalid data provided"));

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.error").value("Bad Request"))
                    .andExpect(jsonPath("$.message").value("Invalid data provided"))
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 400 when bad request exception occurs")
        void createAddressShouldReturn400WhenBadRequestException() throws Exception {
            AddressDto requestDto = createAddressDto();
            String errorMessage = "Invalid data provided";

            when(addressService.createAddress(any(Address.class)))
                    .thenThrow(new BadRequestException(errorMessage));

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.error").value("Bad Request"))
                    .andExpect(jsonPath("$.message").value(errorMessage))
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 500 when internal server error occurs")
        void getAddressShouldReturn500WhenInternalServerError() throws Exception {
            Long addressId = 1L;

            when(addressService.getAddressById(addressId))
                    .thenThrow(new RuntimeException("Database connection failed"));

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isInternalServerError())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(500))
                    .andExpect(jsonPath("$.error").value("Internal Server Error"))
                    .andExpect(jsonPath("$.message").value("An unexpected error occurred"))
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + addressId))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should return 405 when HTTP method not allowed")
        void shouldReturn405WhenMethodNotAllowed() throws Exception {
            mockMvc.perform(patch("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{}"))
                    .andExpect(status().isMethodNotAllowed())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(405))
                    .andExpect(jsonPath("$.error").value("Method Not Allowed"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.timestamp").exists());
        }

        @Test
        @DisplayName("Should return 406 when accept header is not supported")
        void shouldReturn406WhenAcceptHeaderNotSupported() throws Exception {
            Long addressId = 1L;
            AddressDto responseDto = createAddressDto();
            responseDto.setId(addressId);

            when(addressService.getAddressById(addressId)).thenReturn(createMockEntity());

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.TEXT_XML))
                    .andExpect(status().isNotAcceptable())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(406))
                    .andExpect(jsonPath("$.error").value("Not Acceptable"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + addressId))
                    .andExpect(jsonPath("$.timestamp").exists());
        }

        @Test
        @DisplayName("Should handle constraint violation exceptions properly")
        void createAddressShouldHandleConstraintViolationException() throws Exception {
            AddressDto requestDto = createAddressDto();

            when(addressService.createAddress(any(Address.class)))
                    .thenThrow(new ConstraintViolationException("Constraint violation", null));

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.error").value("Bad Request"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).createAddress(any(Address.class));
        }
    }

    @Nested
    @DisplayName("Status Code Tests")
    class StatusCodeTests {

        @Test
        @DisplayName("Should return 201 Created with proper Location header on successful creation")
        void createAddressShouldReturn201WithLocationHeader() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            responseDto.setId(1L);

            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andExpect(header().exists("Location"))
                    .andExpect(header().string("Location", containsString("1")))
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON));

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 200 OK on successful retrieval")
        void getAddressByIdShouldReturn200() throws Exception {
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            when(addressService.getAddressById(addressId)).thenReturn(createMockEntity());

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON));

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should return 200 OK on successful update")
        void updateAddressShouldReturn200() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            when(addressService.updateAddress(eq(addressId), any(Address.class)))
                    .thenReturn(createMockEntity());

            mockMvc.perform(put("/api/addresses/{id}", addressId)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON));

            verify(addressService).updateAddress(eq(addressId), any(Address.class));
        }

        @Test
        @DisplayName("Should return 204 No Content on successful deletion")
        void deleteAddressShouldReturn204() throws Exception {
            Long addressId = 1L;

            doNothing().when(addressService).deleteAddress(addressId);

            mockMvc.perform(delete("/api/addresses/{id}", addressId)
                    .with(csrf()))
                    .andExpect(status().isNoContent())
                    .andExpect(content().string(""));

            verify(addressService).deleteAddress(addressId);
        }

        @Test
        @DisplayName("Should return 404 Not Found when entity does not exist")
        void getAddressByIdShouldReturn404WhenNotFound() throws Exception {
            Long nonExistentId = 999L;

            when(addressService.getAddressById(nonExistentId))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + nonExistentId));

            mockMvc.perform(get("/api/addresses/{id}", nonExistentId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.error").value("Not Found"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).getAddressById(nonExistentId);
        }

        @Test
        @DisplayName("Should return 400 Bad Request on validation failure")
        void createAddressWithInvalidDataShouldReturn400() throws Exception {
            AddressDto invalidDto = AddressDto.builder().build();

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(invalidDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").value("Validation failed"))
                    .andExpect(jsonPath("$.errors").isArray())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any());
        }

        @Test
        @DisplayName("Should return 415 Unsupported Media Type for wrong content type")
        void createAddressWithWrongContentTypeShouldReturn415() throws Exception {
            AddressDto requestDto = createAddressDto();

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.TEXT_PLAIN)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isUnsupportedMediaType())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(415))
                    .andExpect(jsonPath("$.error").value("Unsupported Media Type"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any());
        }

        @Test
        @DisplayName("Should return 500 Internal Server Error on service exception")
        void getAddressByIdShouldReturn500OnServiceException() throws Exception {
            Long addressId = 1L;

            when(addressService.getAddressById(addressId))
                    .thenThrow(new RuntimeException("Database connection failed"));

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isInternalServerError())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(500))
                    .andExpect(jsonPath("$.error").value("Internal Server Error"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should return 409 Conflict on duplicate resource creation")
        void createAddressWithDuplicateDataShouldReturn409() throws Exception {
            AddressDto requestDto = createAddressDto();

            when(addressService.createAddress(any(Address.class)))
                    .thenThrow(new BadRequestException("Address already exists"));

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").value("Address already exists"))
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).createAddress(any(Address.class));
        }


    }

    @Nested
    @DisplayName("Edge Case Tests")
    class EdgeCaseTests {

        @Test
        @DisplayName("Should return 400 when ID is negative")
        void getAddressWithNegativeIdShouldReturnBadRequest() throws Exception {
            Long invalidId = -1L;

            mockMvc.perform(get("/api/addresses/{id}", invalidId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).getAddressById(any());
        }

        @Test
        @DisplayName("Should return 400 when ID is zero")
        void getAddressWithZeroIdShouldReturnBadRequest() throws Exception {
            Long invalidId = 0L;

            mockMvc.perform(get("/api/addresses/{id}", invalidId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).getAddressById(any());
        }

        @Test
        @DisplayName("Should return 400 when request body is null")
        void createAddressWithNullBodyShouldReturnBadRequest() throws Exception {
            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 413 when payload is too large")
        void createAddressWithLargePayloadShouldReturnPayloadTooLarge() throws Exception {
            AddressDto requestDto = createAddressDto();
            requestDto.setStreet("a".repeat(10000)); // Very large string

            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 400 when JSON contains only unknown fields and entity has strict validation")
        void createAddressWithUnknownFieldsShouldReturnBadRequest() throws Exception {
            String jsonWithUnknownFields = """
                {
                    "unknownField": "value",
                    "anotherUnknownField": 123
                }
                """;

            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(jsonWithUnknownFields))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return 400 when JSON has wrong data types")
        void createAddressWithWrongDataTypesShouldReturnBadRequest() throws Exception {
            String jsonWithWrongTypes = """
                {
                    "street": 123,
                }
                """;

            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(jsonWithWrongTypes))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should handle empty list responses correctly")
        void getAllAddresssWhenEmptyListShouldReturnOk() throws Exception {
            List<Address> emptyAddressList = List.of();

            when(addressService.getAllAddresss()).thenReturn(emptyAddressList);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(0)))
                    .andExpect(jsonPath("$").isArray());

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle very large ID values")
        void getAddressWithVeryLargeIdShouldHandleCorrectly() throws Exception {
            Long veryLargeId = Long.MAX_VALUE;

            when(addressService.getAddressById(veryLargeId))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + veryLargeId));

            mockMvc.perform(get("/api/addresses/{id}", veryLargeId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService).getAddressById(veryLargeId);
        }

        @Test
        @DisplayName("Should handle special characters in string fields")
        void createAddressWithSpecialCharactersShouldWork() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            String specialCharsValue = "Oussama's café & résumé (2024) - 100% tested!";
            requestDto.setStreet(specialCharsValue);
            responseDto.setStreet(specialCharsValue);
            responseDto.setId(1L);

            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(header().exists("Location"));

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should handle Unicode characters correctly")
        void createAddressWithUnicodeCharactersShouldWork() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            String unicodeValue = "Mohammed العربية 中文 🚀 ñáéíóú";
            requestDto.setStreet(unicodeValue);
            responseDto.setStreet(unicodeValue);
            responseDto.setId(1L);

            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(header().exists("Location"));

            verify(addressService).createAddress(any(Address.class));
        }
    }

    @Nested
    @DisplayName("HTTP Method and Path Tests")
    class HttpMethodAndPathTests {


        @Test
        @DisplayName("Should return 405 for unsupported HTTP methods on existing paths")
        void unsupportedHttpMethodShouldReturn405() throws Exception {
            mockMvc.perform(patch("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isMethodNotAllowed())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(405))
                    .andExpect(jsonPath("$.error").value("Method Not Allowed"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.timestamp").exists());
        }

        @Test
        @DisplayName("Should return 405 when using PATCH on specific resource")
        void patchOnSpecificResourceShouldReturn405() throws Exception {
            Long addressId = 1L;

            mockMvc.perform(patch("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isMethodNotAllowed())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(405))
                    .andExpect(jsonPath("$.error").value("Method Not Allowed"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());
        }

        @Test
        @DisplayName("Should return 200 when using HEAD on collection (Spring Boot allows HEAD for GET)")
        void headOnCollectionShouldReturn200() throws Exception {
            List<Address> mockEntities = Arrays.asList(createMockEntity());
            when(addressService.getAllAddresss()).thenReturn(mockEntities);

            mockMvc.perform(head("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should return 200 when using OPTIONS on specific resource (Spring Boot handles OPTIONS)")
        void optionsOnSpecificResourceShouldReturn200() throws Exception {
            Long addressId = 1L;

            mockMvc.perform(options("/api/addresses/{id}", addressId))
                    .andExpect(status().isOk())
                    .andExpect(header().exists("Allow"));
        }

        @Test
        @DisplayName("Should handle trailing slashes correctly")
        void pathWithTrailingSlashShouldWork() throws Exception {
            AddressDto addressDto1 = createAddressDto();
            AddressDto addressDto2 = createAddressDto();
            addressDto1.setId(1L);
            addressDto2.setId(2L);

            Address entity1 = createAddress();
            Address entity2 = createAddress();
            entity1.setId(1L);
            entity2.setId(2L);
            List<Address> entityList = Arrays.asList(entity1, entity2);
            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses/")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(2)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle case sensitivity in paths correctly")
        void pathCaseSensitivityShouldReturn404() throws Exception {
            mockMvc.perform(get("/api/ADDRESSS")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isInternalServerError())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(500))
                    .andExpect(jsonPath("$.error").value("Internal Server Error"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());
        }

        @Test
        @DisplayName("Should handle double slashes in paths gracefully")
        void pathWithDoubleSlashesShouldNormalizePath() throws Exception {
            when(addressService.getAllAddresss()).thenReturn(Arrays.asList(createMockEntity()));

            mockMvc.perform(get("/api//addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle missing Accept header gracefully")
        void requestWithoutAcceptHeaderShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should return 406 for unsupported Accept header")
        void requestWithWrongAcceptHeaderShouldReturn406() throws Exception {
            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.TEXT_XML))
                    .andExpect(status().isNotAcceptable())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(406))
                    .andExpect(jsonPath("$.error").value("Not Acceptable"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle multiple Accept headers correctly")
        void requestWithMultipleAcceptHeadersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }
    }

    @Nested
    @DisplayName("JSON Response Structure Tests")
    class JsonResponseStructureTests {

        @Test
        @DisplayName("Should return consistent error response structure for validation failures")
        void validationErrorShouldHaveConsistentStructure() throws Exception {
            AddressDto invalidDto = AddressDto.builder().build();

            mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(invalidDto)))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.timestamp").exists())
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.error").value("Bad Request"))
                    .andExpect(jsonPath("$.message").value("Validation failed"))
                    .andExpect(jsonPath("$.path").value("/api/addresses"))
                    .andExpect(jsonPath("$.errors").isArray())
                    .andExpect(jsonPath("$.errors[0].field").exists())
                    .andExpect(jsonPath("$.errors[0].message").exists());

            verify(addressService, never()).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return consistent error response structure for not found errors")
        void notFoundErrorShouldHaveConsistentStructure() throws Exception {
            Long nonExistentId = 999L;

            when(addressService.getAddressById(nonExistentId))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + nonExistentId));

            mockMvc.perform(get("/api/addresses/{id}", nonExistentId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isNotFound())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.timestamp").exists())
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.error").value("Not Found"))
                    .andExpect(jsonPath("$.message").value("Address not found with id: " + nonExistentId))
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + nonExistentId))
                    .andExpect(jsonPath("$.errors").doesNotExist());

            verify(addressService).getAddressById(nonExistentId);
        }

        @Test
        @DisplayName("Should return consistent error response structure for internal server errors")
        void internalServerErrorShouldHaveConsistentStructure() throws Exception {
            Long addressId = 1L;

            when(addressService.getAddressById(addressId))
                    .thenThrow(new RuntimeException("Database connection failed"));

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isInternalServerError())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.timestamp").exists())
                    .andExpect(jsonPath("$.status").value(500))
                    .andExpect(jsonPath("$.error").value("Internal Server Error"))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.path").value("/api/addresses/" + addressId))
                    .andExpect(jsonPath("$.errors").doesNotExist());

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should return proper JSON structure for successful creation")
        void successfulCreationShouldHaveProperJsonStructure() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            responseDto.setId(1L);

            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.id").value(1))
                    .andExpect(jsonPath("$.street").exists())
                    .andExpect(jsonPath("$.city").exists())
                    .andExpect(jsonPath("$.zipCode").exists())
                    .andExpect(jsonPath("$.timestamp").doesNotExist())
                    .andExpect(jsonPath("$.status").doesNotExist())
                    .andExpect(jsonPath("$.error").doesNotExist());

            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should return proper JSON structure for successful retrieval")
        void successfulRetrievalShouldHaveProperJsonStructure() throws Exception {
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            when(addressService.getAddressById(addressId)).thenReturn(createMockEntity());

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.id").value(1))
                    .andExpect(jsonPath("$.street").exists())
                    .andExpect(jsonPath("$.city").exists())
                    .andExpect(jsonPath("$.zipCode").exists())
                    .andExpect(jsonPath("$.timestamp").doesNotExist())
                    .andExpect(jsonPath("$.status").doesNotExist())
                    .andExpect(jsonPath("$.error").doesNotExist());

            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should return proper JSON array structure for list retrieval")
        void listRetrievalShouldHaveProperJsonStructure() throws Exception {
            AddressDto addressDto1 = createAddressDto();
            AddressDto addressDto2 = createAddressDto();
            addressDto1.setId(1L);
            addressDto2.setId(2L);

            Address entity1 = createAddress();
            Address entity2 = createAddress();
            entity1.setId(1L);
            entity2.setId(2L);
            List<Address> entityList = Arrays.asList(entity1, entity2);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$").isArray())
                    .andExpect(jsonPath("$", hasSize(2)))
                    .andExpect(jsonPath("$[0].id").value(1))
                    .andExpect(jsonPath("$[1].id").value(2))
                    .andExpect(jsonPath("$[0].street").exists())
                    .andExpect(jsonPath("$[1].street").exists())
                    .andExpect(jsonPath("$[0].timestamp").doesNotExist())
                    .andExpect(jsonPath("$[0].status").doesNotExist())
                    .andExpect(jsonPath("$[0].error").doesNotExist());

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should return empty array with proper structure when no entities exist")
        void emptyListShouldHaveProperJsonStructure() throws Exception {
            List<Address> emptyAddressList = List.of();

            when(addressService.getAllAddresss()).thenReturn(emptyAddressList);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$").isArray())
                    .andExpect(jsonPath("$", hasSize(0)))
                    .andExpect(content().json("[]"));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should return no content body for successful deletion")
        void successfulDeletionShouldHaveNoContent() throws Exception {
            Long addressId = 1L;

            doNothing().when(addressService).deleteAddress(addressId);

            mockMvc.perform(delete("/api/addresses/{id}", addressId))
                    .andExpect(status().isNoContent())
                    .andExpect(content().string(""))
                    .andExpect(header().doesNotExist("Content-Type"));

            verify(addressService).deleteAddress(addressId);
        }

        @Test
        @DisplayName("Should handle null values in JSON response correctly")
        void nullValuesInResponseShouldBeHandledCorrectly() throws Exception {
            Long addressId = 1L;

            Address mockEntity = createMockEntity();

            when(addressService.getAddressById(addressId)).thenReturn(mockEntity);

            mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.id").value(1));

            verify(addressService).getAddressById(addressId);
        }
    }

    @Nested
    @DisplayName("Header and Parameter Tests")
    class HeaderAndParameterTests {

        @Test
        @DisplayName("Should handle custom headers correctly")
        void requestWithCustomHeadersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .header("X-Request-ID", "test-request-123")
                    .header("X-Client-Version", "1.0.0")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle query parameters correctly")
        void requestWithQueryParametersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .param("sort", "id")
                    .param("order", "asc")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle pagination parameters correctly")
        void requestWithPaginationParametersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);
            Page<Address> addressPage = new PageImpl<>(entityList, PageRequest.of(0, 10), 1);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(addressPage);

            mockMvc.perform(get("/api/addresses/paginated")
                    .param("page", "0")
                    .param("size", "10")
                    .param("sort", "id,asc")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.content", hasSize(1)))
                    .andExpect(jsonPath("$.totalElements").value(1))
                    .andExpect(jsonPath("$.size").value(10))
                    .andExpect(jsonPath("$.number").value(0));

            verify(addressService).getAllAddresss(any(Pageable.class));
        }



        @Test
        @DisplayName("Should handle very large page size parameters")
        void requestWithLargePageSizeShouldReturnBadRequest() throws Exception {
            mockMvc.perform(get("/api/addresses/paginated")
                    .param("page", "0")
                    .param("size", "10000")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.message").exists())
                    .andExpect(jsonPath("$.timestamp").exists());

            verify(addressService, never()).getAllAddresss(any(Pageable.class));
        }

        @Test
        @DisplayName("Should handle missing required headers gracefully")
        void requestWithoutRequiredHeadersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle malformed query parameters")
        void requestWithMalformedQueryParametersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .param("invalid_param", "value")
                    .param("malformed_param", "")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should handle special characters in query parameters")
        void requestWithSpecialCharactersInParametersShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .param("search", "Oussama & Hicham's café")
                    .param("filter", "name=Mohammed,age>25")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should return proper Content-Type header")
        void responseShouldHaveProperContentTypeHeader() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE))
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }



        @Test
        @DisplayName("Should handle multiple values for same query parameter")
        void requestWithMultipleParameterValuesShouldWork() throws Exception {
            AddressDto addressDto = createAddressDto();
            addressDto.setId(1L);

            Address entity = createAddress();
            entity.setId(1L);
            List<Address> entityList = Arrays.asList(entity);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            mockMvc.perform(get("/api/addresses")
                    .param("tags", "tag1", "tag2", "tag3")
                    .param("categories", "cat1", "cat2")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$", hasSize(1)));

            verify(addressService).getAllAddresss();
        }
    }

    @Nested
    @DisplayName("Pagination Tests")
    class PaginationTests {

        @Test
        @DisplayName("Should return paginated Addresss with default parameters and proper headers")
        void getAddresssWithDefaultPaginationShouldReturnOk() throws Exception {
            AddressDto addressDto1 = createAddressDto();
            AddressDto addressDto2 = createAddressDto();
            addressDto1.setId(1L);
            addressDto2.setId(2L);

            Address entity1 = createAddress();
            Address entity2 = createAddress();
            entity1.setId(1L);
            entity2.setId(2L);
            List<Address> addressList = Arrays.asList(entity1, entity2);
            Page<Address> addressPage = new PageImpl<>(addressList, PageRequest.of(0, 20), 2);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(addressPage);

            mockMvc.perform(get("/api/addresses/paginated")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE))
                    .andExpect(header().string("X-Total-Count", "2"))
                    .andExpect(jsonPath("$.content", hasSize(2)))
                    .andExpect(jsonPath("$.totalElements").value(2))
                    .andExpect(jsonPath("$.totalPages").value(1))
                    .andExpect(jsonPath("$.size").value(20))
                    .andExpect(jsonPath("$.number").value(0))
                    .andExpect(jsonPath("$.first").value(true))
                    .andExpect(jsonPath("$.last").value(true))
                    .andExpect(jsonPath("$.content[0].id").value(1))
                    .andExpect(jsonPath("$.content[1].id").value(2));

            verify(addressService).getAllAddresss(any(Pageable.class));
        }

        @Test
        @DisplayName("Should return paginated Addresss with custom page and size")
        void getAddresssWithCustomPaginationShouldReturnOk() throws Exception {
            Address testAddress = createAddress();
            testAddress.setId(1L);

            List<Address> addressList = Arrays.asList(testAddress);
            Page<Address> addressPage = new PageImpl<>(addressList, PageRequest.of(1, 5), 10);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(addressPage);

            mockMvc.perform(get("/api/addresses/paginated")
                    .param("page", "1")
                    .param("size", "5"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.content", hasSize(1)))
                    .andExpect(jsonPath("$.totalElements").value(10))
                    .andExpect(jsonPath("$.totalPages").value(2))
                    .andExpect(jsonPath("$.size").value(5))
                    .andExpect(jsonPath("$.number").value(1))
                    .andExpect(jsonPath("$.content[0].id").value(1));

            verify(addressService).getAllAddresss(any(Pageable.class));
        }

        @Test
        @DisplayName("Should return paginated Addresss with sorting")
        void getAddresssWithSortingShouldReturnOk() throws Exception {
            Address address1 = createAddress();
            Address address2 = createAddress();
            address1.setId(1L);
            address2.setId(2L);

            List<Address> addressList = Arrays.asList(address2, address1);
            Page<Address> addressPage = new PageImpl<>(addressList, PageRequest.of(0, 20), 2);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(addressPage);

            mockMvc.perform(get("/api/addresses/paginated")
                    .param("sort", "id,desc"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.content", hasSize(2)))
                    .andExpect(jsonPath("$.totalElements").value(2))
                    .andExpect(jsonPath("$.content[0].id").value(2))
                    .andExpect(jsonPath("$.content[1].id").value(1));

            verify(addressService).getAllAddresss(any(Pageable.class));
        }

        @Test
        @DisplayName("Should return empty page when no Addresss exist")
        void getAddresssWhenEmptyShouldReturnEmptyPage() throws Exception {
            Page<Address> emptyAddressPage = new PageImpl<>(Arrays.asList(), PageRequest.of(0, 20), 0);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(emptyAddressPage);

            mockMvc.perform(get("/api/addresses/paginated"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.content", hasSize(0)))
                    .andExpect(jsonPath("$.totalElements").value(0))
                    .andExpect(jsonPath("$.totalPages").value(0))
                    .andExpect(jsonPath("$.size").value(20))
                    .andExpect(jsonPath("$.number").value(0));

            verify(addressService).getAllAddresss(any(Pageable.class));
        }



        @Test
        @DisplayName("Should handle large page size requests")
        void getAddresssWithLargePageSizeShouldReturnOk() throws Exception {
            List<Address> addressList = Arrays.asList();
            Page<Address> addressPage = new PageImpl<>(addressList, PageRequest.of(0, 1000), 0);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(addressPage);

            mockMvc.perform(get("/api/addresses/paginated")
                    .param("size", "1000"))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.size").value(1000));

            verify(addressService).getAllAddresss(any(Pageable.class));
        }
    }

    @Nested
    @DisplayName("OpenAPI/Swagger Documentation Tests")
    class SwaggerDocumentationTests {

        @Test
        @DisplayName("Should have proper OpenAPI annotations on create endpoint")
        void createAddressEndpointShouldHaveProperSwaggerAnnotations() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            responseDto.setId(1L);

            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            MvcResult result = mockMvc.perform(post("/api/addresses")
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andReturn();

            // @Operation(summary = "Create Address", description = "Creates a new Address")
            // @ApiResponses({@ApiResponse(responseCode = "201", description = "Created")})
            assertThat(result.getResponse().getStatus()).isEqualTo(201);
            verify(addressService).createAddress(any(Address.class));
        }

        @Test
        @DisplayName("Should have proper OpenAPI annotations on get by ID endpoint")
        void getAddressByIdEndpointShouldHaveProperSwaggerAnnotations() throws Exception {
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            when(addressService.getAddressById(addressId)).thenReturn(createMockEntity());

            MvcResult result = mockMvc.perform(get("/api/addresses/{id}", addressId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andReturn();

            // Verify that the endpoint would be documented with proper OpenAPI annotations
            // @Operation(summary = "Get Address by ID", description = "Returns a single Address")
            // @ApiResponses({
            //     @ApiResponse(responseCode = "200", description = "OK"),
            //     @ApiResponse(responseCode = "404", description = "Not Found")
            // })
            assertThat(result.getResponse().getStatus()).isEqualTo(200);
            verify(addressService).getAddressById(addressId);
        }

        @Test
        @DisplayName("Should have proper OpenAPI annotations on update endpoint")
        void updateAddressEndpointShouldHaveProperSwaggerAnnotations() throws Exception {
            AddressDto requestDto = createAddressDto();
            AddressDto responseDto = createAddressDto();
            Long addressId = 1L;
            responseDto.setId(addressId);

            when(addressService.updateAddress(eq(addressId), any(Address.class)))
                    .thenReturn(createMockEntity());

            MvcResult result = mockMvc.perform(put("/api/addresses/{id}", addressId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isOk())
                    .andReturn();

            // Verify that the endpoint would be documented with proper OpenAPI annotations
            // @Operation(summary = "Update Address", description = "Updates an existing Address")
            // @ApiResponses({
            //     @ApiResponse(responseCode = "200", description = "OK"),
            //     @ApiResponse(responseCode = "404", description = "Not Found")
            // })
            assertThat(result.getResponse().getStatus()).isEqualTo(200);
            verify(addressService).updateAddress(eq(addressId), any(Address.class));
        }

        @Test
        @DisplayName("Should have proper OpenAPI annotations on delete endpoint")
        void deleteAddressEndpointShouldHaveProperSwaggerAnnotations() throws Exception {
            Long addressId = 1L;

            doNothing().when(addressService).deleteAddress(addressId);

            MvcResult result = mockMvc.perform(delete("/api/addresses/{id}", addressId))
                    .andExpect(status().isNoContent())
                    .andReturn();

            // Verify that the endpoint would be documented with proper OpenAPI annotations
            // @Operation(summary = "Delete Address", description = "Deletes a Address")
            // @ApiResponses({
            //     @ApiResponse(responseCode = "204", description = "No Content"),
            //     @ApiResponse(responseCode = "404", description = "Not Found")
            // })
            assertThat(result.getResponse().getStatus()).isEqualTo(204);
            verify(addressService).deleteAddress(addressId);
        }

        @Test
        @DisplayName("Should have proper OpenAPI annotations on get all endpoint")
        void getAllAddresssEndpointShouldHaveProperSwaggerAnnotations() throws Exception {
            AddressDto addressDto1 = createAddressDto();
            AddressDto addressDto2 = createAddressDto();
            addressDto1.setId(1L);
            addressDto2.setId(2L);

            Address entity1 = createAddress();
            Address entity2 = createAddress();
            entity1.setId(1L);
            entity2.setId(2L);
            List<Address> entityList = Arrays.asList(entity1, entity2);

            when(addressService.getAllAddresss()).thenReturn(entityList);

            MvcResult result = mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andReturn();

            // Verify that the endpoint would be documented with proper OpenAPI annotations
            // @Operation(summary = "Get all Addresss", description = "Returns a list of all Addresss")
            // @ApiResponse(responseCode = "200", description = "OK")
            assertThat(result.getResponse().getStatus()).isEqualTo(200);
            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should have proper OpenAPI annotations on paginated endpoint")
        void getAddresssPageEndpointShouldHaveProperSwaggerAnnotations() throws Exception {
            AddressDto addressDto1 = createAddressDto();
            AddressDto addressDto2 = createAddressDto();
            addressDto1.setId(1L);
            addressDto2.setId(2L);

            Address entity1 = createAddress();
            Address entity2 = createAddress();
            entity1.setId(1L);
            entity2.setId(2L);
            List<Address> entityList = Arrays.asList(entity1, entity2);
            Page<Address> addressPage = new PageImpl<>(entityList, PageRequest.of(0, 20), 2);

            when(addressService.getAllAddresss(any(Pageable.class))).thenReturn(addressPage);

            MvcResult result = mockMvc.perform(get("/api/addresses/paginated")
                    .param("page", "0")
                    .param("size", "20")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andReturn();

            // Verify that the endpoint would be documented with proper OpenAPI annotations
            // @Operation(summary = "Get paginated Addresss", description = "Returns a paginated list of Addresss")
            // @Parameter(name = "page", description = "Page number (0-based)")
            // @Parameter(name = "size", description = "Page size")
            // @Parameter(name = "sort", description = "Sort criteria")
            // @ApiResponse(responseCode = "200", description = "OK")
            assertThat(result.getResponse().getStatus()).isEqualTo(200);
            verify(addressService).getAllAddresss(any(Pageable.class));
        }

        @Test
        @DisplayName("Should document error responses properly")
        void errorResponsesShouldBeProperlyDocumented() throws Exception {
            Long nonExistentId = 999L;

            when(addressService.getAddressById(nonExistentId))
                    .thenThrow(new EntityNotFoundException("Address not found with id: " + nonExistentId));

            MvcResult result = mockMvc.perform(get("/api/addresses/{id}", nonExistentId)
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isNotFound())
                    .andReturn();

            // Verify that error responses would be documented with proper OpenAPI annotations
            // @ApiResponse(responseCode = "404", description = "Not Found", 
            //              content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
            assertThat(result.getResponse().getStatus()).isEqualTo(404);
            verify(addressService).getAddressById(nonExistentId);
        }

        @Test
        @DisplayName("Should document validation error responses")
        void validationErrorResponsesShouldBeProperlyDocumented() throws Exception {
            AddressDto invalidDto = AddressDto.builder().build(); // Invalid DTO

            MvcResult result = mockMvc.perform(post("/api/addresses")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(invalidDto)))
                    .andExpect(status().isBadRequest())
                    .andReturn();

            // Verify that validation error responses would be documented with proper OpenAPI annotations
            // @ApiResponse(responseCode = "400", description = "Bad Request - Validation Error",
            //              content = @Content(schema = @Schema(implementation = ValidationErrorResponse.class)))
            assertThat(result.getResponse().getStatus()).isEqualTo(400);
            verify(addressService, never()).createAddress(any(Address.class));
        }
    }

    @Nested
    @DisplayName("Security and Authorization Tests")
    class SecurityAndAuthorizationTests {

        @Test
        @DisplayName("Should allow access to public endpoints without authentication")
        void accessPublicEndpointWithoutAuthenticationShouldSucceed() throws Exception {
            List<Address> mockEntities = Arrays.asList(createMockEntity());
            when(addressService.getAllAddresss()).thenReturn(mockEntities);

            mockMvc.perform(get("/api/addresses")
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$").isArray())
                    .andExpect(jsonPath("$[0].id").exists());

            verify(addressService).getAllAddresss();
        }

        @Test
        @DisplayName("Should allow create operation for any user since security is disabled")
        void createWithUserRoleShouldSucceed() throws Exception {
            AddressDto requestDto = createAddressDto();
            when(addressService.createAddress(any(Address.class))).thenReturn(createMockEntity());

            mockMvc.perform(post("/api/addresses")
                    .with(user("testuser").roles("USER"))
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(requestDto)))
                    .andExpect(status().isCreated())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.id").value(1));

            verify(addressService).createAddress(any(Address.class));
        }

    }




}
